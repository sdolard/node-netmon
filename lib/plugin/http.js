...
// TODO: http://en.wikipedia.org/wiki/HTTP_location
RE_LOCATION_HOST= /(^http[s]?:\/\/)?([\w.]+)(:\d+)?[\w\W]*$/i;
...
function check (id, config, callback, progressLog) {
	...
	request = httpF.get({
		...
		progressLog(util.format('Connected: %s', sanResponse.statusMessage));
	...
	request.setTimeout(config.timeout * 1000, function(){
			...
			try {
				...
				console.err('!!!!!! request.destroy() throw an error!');
...
exports.run = check;
exports.RE_LOCATION_HOST = RE_LOCATION_HOST;