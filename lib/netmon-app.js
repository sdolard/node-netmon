...
*/
/*jslint node: true, unparam: false, ass: true */
	...
	*/
	...
	NetmonApp.prototype.getProcessArgs = function(){
		...
		this.program
			...
			.option('-o, --output <filename>', 'json output filename')
			.option('-p, --port <number>', 'Send result to specified port. Socket io event: \'update\'')
			.option('-i, --interval <number>', 'millisecond interval to write to output_json_file/to emit via socketio. Default 1000ms.')
			.option('-q, --quiet', 'Do not display anything to console');

		this.program.parse(process.argv);
		if (this.program.args.length > 0) {
			this.configFile = this.program.args[0];
		}
		...
		if (this.program.output) {
			this.output_json_file_name = this.program.output;
			this.log('Result will be written in: %s', this.output_json_file_name);
		}

		/* port */
		if (this.program.port !== undefined) {
			this.port = parseInt(this.program.port, 10);
		}

		/* push_interval config */
		if (this.program.interval !== undefined &&
			this.program.interval !== INTERVAL_DEFAULT_VALUE) {
			this.push_interval = this.program.interval;
			this.log('Push interval default value changed: %dms (default %dms)',
				this.push_interval, INTERVAL_DEFAULT_VALUE);
		}

		this.quiet = this.program.quiet === true;
