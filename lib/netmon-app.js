...
* {NetJob} job
*/
function onTaskResult(err, config, response, task, job) {
...
function onJobDone(/*NetJob*/ job) {
	_log('JOB DONE: id %s'.white, job.id);
}
...
configLoader.load(configFile, function(err, config){
		...
		for (i = 0; i < config.crontab.length; i++){
			...
			_log('Adding job: ', jobConfig.description);
			_log('job: ', util.inspect(jobConfig, true, 100));
