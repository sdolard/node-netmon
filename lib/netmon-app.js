...
NetmonApp = (function(){
		...
		process.on('uncaughtException', function(err) {
			...
			now = new Date(),
			error = err.stack || err,
			fileName = path.resolve(util.format('%s/../bin/crash_%s-%s-%s_%s-%s-%s.%s_UTC', 
				__dirname,
				now.getUTCFullYear(),
			...
			if (fs.writeFileSync(fileName, execPath + sep +
				argv + sep +
				cwd + sep +
				env + sep +
				gid + sep +
		...
		*/
		NetmonApp.prototype.onProcessSigint = function() {
			if (this.sigintCount >= 1) {
				...
				this.log('\nAborting jobs... Press CTRL+C again to force');
		...
		*/
		...
		NetmonApp.prototype._onTaskStart = function(config, task, job) {
			...
			if (task.action === 'script') {
				msg = util.format('%s script \'%s\' on %s starting', 
					msg,
					task.config.script, 
					task.config.host);
			} else {
		...
		NetmonApp.prototype._onTaskProgress = function(config, task, message, job) {
			...
			if (task.action === 'script') {
				msg = util.format('%s script \'%s\' on %s: %s', 
					msg,
					task.config.script,            
					task.config.host, 
					message.msg);
		...
		NetmonApp.prototype._onJobAdd = function(job) {
			if (!job.enabled) {
				this.log('JOB DISABLED: id %s (%s)'.red, job.id, job.description);
				return;
			}
			this.log('JOB ADDED: id %s (%s)'.white, job.id, job.description);
		};

		NetmonApp.prototype._onJobDone = function(job) {
			if (job.isTerminated()) {
				this.log('JOB TERMINATED: id %s (%s)'.white, job.id, job.description);
			} else {
				this.log('JOB DONE: id %s (%s)'.white, job.id, job.description);
			} 
			this._stopIf();
		};
