...
NetJobManager = (function() {
	...
	*/
	NetJobManager.prototype.load = function (filename, callback) {
		...
		configLoader.load(filename, function(err, config){
				...
				if (!config.crontab) {
					this.emit('loaded', new Error('Invalid config file: no crontab property'));
					return;
				}
				
				for (i = 0; i < config.crontab.length; i++){
					this.addJob(netjob.sanitizeConfig(config.crontab[i]));
				}
				this.emit('loaded');
		}.bind(this));	
	};
	...
	*/
	NetJobManager.prototype._onTaskStart = function(config, task, job) {
		...
		// Even if job run tasks, it does not keep unified form/relation to them.
		// (tasks can have many form)
		// This is why NetJobManager exists.
		if (!this._jobs[job.id].hasOwnProperty('task')) {
			this._jobs[job.id].task = {};
		}
		// TODO: review  this ?
		this._jobs[job.id].task[task.id] = this.toDataItem({
				task: task,
				config: config
		});
	...
	*/
	...
	NetJobManager.prototype._onJobDone = function(job) {
		this._jobs[job.id].doneCount = job.getDoneCount();
	...
	NetJobManager.prototype.jobsAreTerminated = function() {
		...
		for (jobId in this._jobs) {
			if (this._jobs.hasOwnProperty(jobId)) {
				jobCount++; // disabled jobs are not added in this.jobs
		...
		return (terminatedJobs > 1) && (terminatedJobs === jobCount);
	};
