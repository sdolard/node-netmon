...
NetworkMonitor.prototype.setMonitors = function (monitors) {
	var monitor, path, port, target;
	for(monitor in  monitors) {
		if (monitors.hasOwnProperty(monitor)) {

			// check if path and/or port is added to target
			if (monitor.indexOf('/') !== -1) {
				path = monitor.split('/');
				path.unshift();
				monitors[monitor].path = path.join('/');
				target = monitor.split('/')[0];
			...
			if (target.indexOf(':') !== -1) {
				monitors[monitor].port = target.split(':')[1];
				target = target.split(':')[0];
			}

			monitors[monitor].target = target;

			// ping monitor
			if (monitors[monitor].type.indexOf('ping') !== -1) {
				console.log('monitor: add ping on ' + target);
				this.monitors.push(this.pingMonitor(monitors[monitor]));
			}

			// http monitor
			if (monitors[monitor].type.indexOf('http') !== -1) {
				console.log('monitor: add http on ' + target);
				this.monitors.push(this.httpMonitor(monitors[monitor]));
			}

			// https monitor
			if (monitors[monitor].type.indexOf('https') !== -1) {
				console.log('monitor: add https on ' + target);
				this.monitors.push(this.httpsMonitor(monitors[monitor]));
			}
		}
	}
};
...
/**
 * internal function to convert config monitor into http monitor
 * @private
 */
NetworkMonitor.prototype.httpMonitor = function (config) {
	...
	return {
		fn: http.check,
		type: 'http',
		args: [cfg, this.sendHttpData]
	};
};

/**
 * internal function to convert config monitor into https monitor
 * @private
 */
NetworkMonitor.prototype.httpsMonitor = function (config) {
	var cfg = {
		host: config.target,
		ssl: true
	};
